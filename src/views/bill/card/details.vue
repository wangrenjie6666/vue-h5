<template>
  <div class="box">
    <div class="my-content">
      <div class="info-box">
        <div class="title-box">
          <van-image class="img" :src="data.icon" :width="25" :height="25" />
          <div class="name">{{ data.classifyName }}</div>
    ...
    <div class="my-footer" v-if="$route.query.billType == 0">
      <van-button
...
<script>
import { cardInfoById, cardBillDelete } from "@/api/write.js";
export default {
  name: "billCardDetails",
  data() {
    return {
  ...
  methods: {
    ...
    handleDel() {
      this.$vDialog
        ...
        .then(() => {
          cardBillDelete({ did: this.query.did }).then((res) => {
            this.$vtoast("删除账单成功");
            this.$router.go(-1);
          });
        })
